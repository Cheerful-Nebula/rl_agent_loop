...
### Physics-derived information

Additional physics metrics are provided through `info["raw_physics"]`:

* `distance_from_origin`: Euclidean distance to (0,0)
* `angular_velocity`: Absolute angular velocity
* `linear_velocity_mag`: Magnitude of linear velocity
* `fuel_consumed_this_step`: Fuel cost from the most recent action

These values are already computed for you—do not recompute them.
...
* **Output code only** — no explanations, comments, or markdown
